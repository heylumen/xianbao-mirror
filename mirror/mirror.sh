...
PREFIX="${PAGES_PREFIX:-/}"
CSSLINK="<link rel=\"stylesheet\" href=\"${PREFIX%/}/lib/xianbao-override.css?v=6\">"
...
# 幂等可重入；配合 CHECKPOINT_EVERY>0（每 120 页提交一次检查点）时，
# 末尾的 rebuild/build_hub 仍保证最终提交的列表/首页处于正确终态。
# ---------------------------------------------------------------------------
...
css_link = '<link rel="stylesheet" href="/lib/xianbao-override.css?v=6">'
marker = "xianbao-override.css"
count = 0
for p in out_dir.rglob("*"):
    if p.suffix.lower() not in (".html", ".htm"):
